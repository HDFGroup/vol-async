...
main(int argc, char *argv[])
{
    ...
    for (i = 0; i < proc_num; i++) {
        ...
        mdset_ids[i] =
            H5Dcreate_async(fid, dname, H5T_NATIVE_INT, fspace, H5P_DEFAULT, dcpl, H5P_DEFAULT, es_id);
        if (mdset_ids[i] < 0) {
    ...
    for (i = 0; i < NSEG; i++) {
        ...
        if (proc_num <= 2)
            fprintf(stderr, "Rank %d: mem %lu %lu, file %lu %lu\n", my_rank, mem_start[0], mem_count[0],
                    file_start[0], file_count[0]);
    }

    for (i = 0; i < proc_num; i++) {
        for (j = 0; j < NSEG; j++) {
            status = H5Dwrite_async(mdset_ids[i], mem_type_ids[j], mem_space_ids[j], file_space_ids[j], dxpl,
                                    write_data, es_id);
            if (status < 0) {
    ...
    /*     } */
    /*     H5Dwrite_multi(NSEG, dset_ids, mem_type_ids, mem_space_ids, file_space_ids, dxpl,
     * (void*)data_mdset); */
    /* } */
    ...
    for (j = 0; j < NSEG; j++) {
        status = H5Dread_async(mdset_ids[my_rank], mem_type_ids[j], mem_space_ids[j], file_space_ids[j], dxpl,
                               read_data, es_id);
        if (status < 0) {
            fprintf(stderr, "Error with dset %d seg %d write\n", i, j);
            MPI_Abort(MPI_COMM_WORLD, -1);

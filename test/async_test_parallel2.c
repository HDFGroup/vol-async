...
int verify (int *data, int size, int multiplier) {
	...
	for (i = 0; i < size; ++i)
		if (data[i] != i * multiplier) return -1;
...
int main (int argc, char *argv[]) {
	...
	if (my_rank == proc_num - 1) {
		if (DIMLEN % proc_num != 0) my_size[0] += DIMLEN % proc_num;
	}
	...
	dset0_id = H5Dcreate (grp_id, "dset0", H5T_NATIVE_INT, fspace_id, H5P_DEFAULT, H5P_DEFAULT,
						  H5P_DEFAULT);
	if (dset0_id < 0) {
	...
	dset1_id = H5Dcreate (grp_id, "dset1", H5T_NATIVE_INT, fspace_id, H5P_DEFAULT, H5P_DEFAULT,
						  H5P_DEFAULT);
	if (dset1_id < 0) {
	...
	H5VLasync_finalize ();
	if (data0_write != NULL) free (data0_write);
	if (data0_read != NULL) free (data0_read);
	if (data1_write != NULL) free (data1_write);
	if (data1_read != NULL) free (data1_read);

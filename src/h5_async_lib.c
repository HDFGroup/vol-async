...
{
    herr_t status;
    status = H5Pinsert2(dxpl, H5VL_ASYNC_DISABLE_IMPLICIT_NAME, sizeof(hbool_t), false, NULL, NULL, NULL,
                        NULL, NULL, NULL);
    if (status < 0) {
...
H5Pset_dxpl_pause(hid_t dxpl, hbool_t is_pause)
{
    herr_t status;
    status =
        H5Pinsert2(dxpl, H5VL_ASYNC_PAUSE_NAME, sizeof(hbool_t), false, NULL, NULL, NULL, NULL, NULL, NULL);
    if (status < 0) {
        fprintf(stderr, "  [ASYNC VOL ERROR] %s: H5Pinsert2 failed!\n", __func__);
        return (-1);
    }

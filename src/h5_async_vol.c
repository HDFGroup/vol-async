...
const char *task_type_names_g[10] = {"QTYPE_NONE", "REGULAR",  "DEPENDENT",
                                     "COLLECTIVE", "BLOCKING", "ISOLATED"};
...
func_enter(const char *func, const char *name)
{
    ...
    if (name) {
        if (async_instance_g &&
            ...
            fprintf(fout_g, "  [%s DBG] %ld.%06ld: entering [%s], push=%d, [%s]\n", type, now.tv_sec,
 ...
 */
static void
free_async_task(async_task_t *task, const char *call_func)
{
    func_enter(__func__, call_func);
 ...
 * \ingroup ASYNC
 *
 * \brief Get the number of tasks in Argobots pool, without locking queue mutex
 *
 * \param[in] task Async task
 *
 * \return int
 *
 * \details Retrieve the number of tasks already pushed into the Argobots pool by iterating through
...
get_n_running_task_in_queue_obj(H5VL_async_t *async_obj, const char *call_func)
{
    ...
    if (ABT_mutex_unlock(async_instance_g->qhead.head_mutex) != ABT_SUCCESS) {
        fprintf(fout_g, "  [ASYNC VOL ERROR] %s with ABT_mutex_unlock head_mutex\n", __func__);
        return -1;
    }
#ifdef ENABLE_DBG_MSG
...
add_task_to_queue(async_qhead_t *qhead, async_task_t *task, task_type_t task_type)
{
    ...
    /*     /1*             task_elt->async_obj == task->async_obj && !(task->op == READ && task_elt->op ==
     * READ)) { *1/ */
...
check_app_acquire_mutex_fn(async_task_t *task, unsigned int *mutex_count, hbool_t *acquired)
{
    ...
    while (async_instance_g->pause) {
        ...
        wait_count++;
        if (wait_count == 10000) {
            fprintf(fout_g, "  [      ABT INFO] async operations are paused for 10 seconds, use "
    ...
    if (H5TSmutex_get_attempt_count(&attempt_count) < 0) {
        fprintf(fout_g, "  [      ABT ERROR] %s H5TSmutex_get_attempt_count failed\n", __func__);
        return -1;
    }
        ...
        while (1) {
            if (task->async_obj->file_async_obj->attempt_check_cnt % ASYNC_ATTEMPT_CHECK_INTERVAL == 0) {
                ...
                if (new_attempt_count <= attempt_count) {
                    ...
                    if (async_instance_g &&
                        (async_instance_g->mpi_rank == ASYNC_DBG_MSG_RANK || -1 == ASYNC_DBG_MSG_RANK))
                        fprintf(fout_g, "  [      ABT DBG] %s counter %d/%d, reset wait time to %d \n",
    ...
    while (*acquired == false && *mutex_count > 0) {
        if (H5TSmutex_acquire(*mutex_count, acquired) < 0) {
            fprintf(fout_g, "  [      ABT ERROR] %s H5TSmutex_acquire failed\n", __func__);
            return -1;
        }
        ...
        if (wait_count > 0) {
            ...
            if (wait_count == 10000) {
                fprintf(fout_g,
                        "  [      ABT INFO] %s unable to acquire HDF5 mutex for 10 seconds, deadlock?\n",
                        __func__);
                wait_count = 1;
            }
        }
...
check_app_wait(int attempt_count, const char *func_name)
{
    ...
    if (attempt_count > 0 && new_attempt_count > attempt_count + op_count) {
        if (async_instance_g->sleep_time < ASYNC_APP_CHECK_SLEEP_TIME_MAX) {
            ...
            if (async_instance_g &&
                (async_instance_g->mpi_rank == ASYNC_DBG_MSG_RANK || -1 == ASYNC_DBG_MSG_RANK))
                fprintf(fout_g, "  [      ABT DBG] %s counter %d/%d, increase wait time to %d \n", func_name,
...
async_attr_create_fn(void *foo)
{
    ...
    if (H5VLOPEN_LIB_CONTEXT(&task->h5_context) < 0) {
        fprintf(fout_g, "  [      ABT ERROR] %s %s failed\n", __func__, FUNC_H5VLOPEN_LIB_CONTEXT_STR);
        goto done;
    }
    if (H5VLrestore_lib_state(task->h5_state) < 0) {
        fprintf(fout_g, "  [      ABT ERROR] %s H5VLrestore_lib_state failed\n", __func__);
        goto done;
    }
    ...
    while (1) {
        if (ABT_mutex_trylock(task->async_obj->obj_mutex) == ABT_SUCCESS) {
            ...
            fprintf(fout_g, "  [      ABT DBG] %s error with try_lock\n", __func__);
            break;
        }
        usleep(1000);
    }
    ...
    if (is_lib_state_restored && H5VLCLOSE_LIB_CONTEXT(task->h5_context) < 0)
        fprintf(fout_g, "  [      ABT ERROR] %s %s failed\n", __func__, FUNC_H5VLCLOSE_LIB_CONTEXT_STR);
    ...
    if (is_lock == 1) {
        if (ABT_mutex_unlock(task->async_obj->obj_mutex) != ABT_SUCCESS)
            fprintf(fout_g, "  [      ABT ERROR] %s ABT_mutex_unlock failed\n", __func__);
    }
                  ...
                  hid_t dxpl_id, void **req)
{
    ...
    if (is_blocking) {
        if (block_and_wait_task(async_task, __func__) < 0)
            goto error;
    }
                   ...
                   hid_t mem_space_id[], hid_t file_space_id[], hid_t plist_id, void *buf[], void **req)
{
    ...
    if (req) {
        H5VL_async_t *new_req;
        if ((new_req = H5VL_async_new_obj(NULL, parent_obj[0]->under_vol_id)) == NULL) {
            fprintf(fout_g, "  [ASYNC VOL ERROR] %s with request object calloc\n", __func__);
            goto error;
        }
        new_req->my_task = async_task;
    ...
    /* Lock parent_obj */
    while (1) {
        if (parent_obj[0]->obj_mutex && ABT_mutex_trylock(parent_obj[0]->obj_mutex) == ABT_SUCCESS) {
            lock_parent = true;
            break;
        }
        // Temp release global lock in case background is waiting
...
static herr_t
async_dataset_read_merge_mdset_col(async_instance_t *aid, size_t count, H5VL_async_t **parent_obj,
                                   ...
                                   hid_t plist_id, void *buf[], void **req)
{
    ...
    return found_task;
} // End async_dataset_read_merge_mdset_col
#endif
...
async_dataset_write_fn(void *foo)
{
    ...
    while (1) {
        if (count > 10000) {
            fprintf(fout_g, "  [      ABT DBG] %s cannot acquire object lock in 10s!\n", __func__);
            break;
        }
    ...
    if (status < 0) {
        if ((task->err_stack = H5Eget_current_stack()) < 0)
            fprintf(fout_g, "  [      ABT ERROR] %s H5Eget_current_stack failed\n", __func__);
        fprintf(fout_g, "  [      ABT LOG] Argobots execute %s failed\n", __func__);
#ifdef PRINT_ERROR_STACK
        H5Eprint2(task->err_stack, stderr);
#endif
        goto done;
            ...
            for (size_t i = iter_cnt; i < total_cnt; i++) {
                ...
                if (async_instance_g->used_mem + buf_size > async_instance_g->max_mem) {
                    ...
                    if (1 != is_contig_memspace(mem_space_id[i])) {
                        ...
                        H5Dgather(mem_space_id[i], buf[i], mem_type_id[i], buf_size, iter_args->buf[i], NULL,
                                  NULL);
                        hsize_t elem_size = H5Tget_size(mem_type_id[i]);
                        if (elem_size == 0)
                            elem_size = 1;
...
async_file_create_fn(void *foo)
{
     ...
     * &supported) < 0) { */
    /*     fprintf(fout_g,"  [      ABT ERROR] %s H5VLintrospect_opt_query failed\n", __func__); */
    /*     goto done; */
    /* } */
    ...
    if (ABT_mutex_lock(async_instance_mutex_g) != ABT_SUCCESS) {
        fprintf(fout_g, "  [      ABT ERROR] with ABT_mutex_lock\n");
        goto done;
    };
    async_instance_g->nfopen++;
    if (ABT_mutex_unlock(async_instance_mutex_g) != ABT_SUCCESS) {
        fprintf(fout_g, "  [      ABT ERROR] with ABT_mutex_ulock\n");
        goto done;
    };
...
H5VL_async_unwrap_object(void *obj)
{
    ...
    if (under)
        H5VL_async_free_obj(o, __func__);
...
H5VL_async_is_implicit_disabled(int op_type, const char *func_name)
{
    ...
    // If implicit is disabled, wait for all existing async task to finish
    /* if (ret_value == 1) { */
    /*     func_log(func_name, "implicit async disabled, wait for existing async tasks"); */
    /*     async_waitall(1); */
    /* } */
                       ...
                       hid_t space_id, hid_t acpl_id, hid_t aapl_id, hid_t dxpl_id, void **req)
{
    ...
    if (H5VL_async_is_implicit_disabled(ATTR_OP, __func__)) {
                        ...
                        hid_t file_space_id[], hid_t plist_id, void *buf[], void **req)
{
    ...
    if (H5VL_async_is_implicit_disabled(DSET_RW_OP, __func__)) {
        ...
        if (xfer_mode == H5FD_MPIO_COLLECTIVE) {
            if ((merged =
                     async_dataset_read_merge_mdset_col(async_instance_g, count, o, mem_type_id, mem_space_id,
                         ...
                         hid_t file_space_id[], hid_t plist_id, const void *buf[], void **req)
{
    ...
    if (H5VL_async_is_implicit_disabled(DSET_RW_OP, __func__)) {
        ...
        ret_value = H5VLdataset_write(count, obj, o[0]->under_vol_id, mem_type_id, mem_space_id,
        ...
        if (xfer_mode == H5FD_MPIO_COLLECTIVE) {
            if ((merged = async_dataset_write_merge_mdset_col(async_instance_g, count, o, mem_type_id,
                                                              mem_space_id, file_space_id, plist_id, buf,
                                                              req)) < 0)
                           ...
                           hid_t lcpl_id, hid_t tcpl_id, hid_t tapl_id, hid_t dxpl_id, void **req)
{
    ...
    if (H5VL_async_is_implicit_disabled(DTYPE_OP, __func__)) {
 ...
 */
    ...
    if (H5VL_async_is_implicit_disabled(FILE_OP, __func__)) {
...
H5VL_async_file_specific(void *file, H5VL_file_specific_args_t *args, hid_t dxpl_id, void **req)
{
    ...
    if (args->op_type == H5VL_FILE_IS_ACCESSIBLE || args->op_type == H5VL_FILE_DELETE) {
        ...
        if (args->op_type == H5VL_FILE_REOPEN || H5VL_async_is_implicit_disabled(FILE_OP, __func__)) {
                        ...
                        hid_t gcpl_id, hid_t gapl_id, hid_t dxpl_id, void **req)
{
    ...
    if (H5VL_async_is_implicit_disabled(GROUP_OP, __func__)) {
...
H5VL_async_group_close(void *grp, hid_t dxpl_id, void **req)
{
    ...
    if (H5VL_async_is_implicit_disabled(GROUP_OP, __func__) || abt_id > 0) {
                       ...
                       void **req)
{
    ...
    if (H5VL_async_is_implicit_disabled(OBJ_OP, __func__)) {
...
H5VL_async_introspect_get_conn_cls(void *obj, H5VL_get_conn_lvl_t lvl, const H5VL_class_t **conn_cls)
{
    ...
    if (H5VL_GET_CONN_LVL_CURR == lvl) {
        *conn_cls = &H5VL_async_g;
        ...
        if (NULL == o->under_object) {
            fprintf(fout_g, "  [ASYNC VOL ERROR] %s, o->under_object is NULL\n", __func__);
            H5VL_async_start();
            H5VL_async_object_wait(o);
        }
        if (0 == o->under_vol_id) {
            fprintf(fout_g, "  [ASYNC VOL ERROR] %s, o->under_vol_id is 0\n", __func__);
        }

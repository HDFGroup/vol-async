                ...
                if( ndim==1){
                   ...
                   /*  fprintf(stderr, "         current_task_start  = [%llux%llu] \n", (unsigned long long)current_task_start_out[0],(unsigned long long)current_task_start_out[1]);
                    ...
                    if(current_task_start_out[1]==iterator_task_start_out[1]){
                        if((current_task_start_out[2]==iterator_task_start_out[2]+iterator_task_count_out[2])&&(current_task_count_out[0]==iterator_task_count_out[0])&&(current_task_start_out[0]==iterator_task_start_out[0]))
                       //fprintf(stderr,"current task start index [%lld] is  contagious to iterator task start=[%lld] and count=[%lld] are contagious",current_task_start_out[0],iterator_task_start_out[0],iterator_task_count_out[0]);
                            {
                                start[0]=iterator_task_start_out[0];
                                start[1]=iterator_task_start_out[1];
                    ...
                    {
                       if((current_task_start_out[1]==iterator_task_start_out[1]+iterator_task_count_out[1])&&(current_task_count_out[0]==iterator_task_count_out[0])&&(current_task_start_out[0]==iterator_task_start_out[0]))
                       //fprintf(stderr,"current task start index [%lld] is  contagious to iterator task start=[%lld] and count=[%lld] are contagious",current_task_start_out[0],iterator_task_start_out[0],iterator_task_count_out[0]);
                        {    
                            start[0]=iterator_task_start_out[0];
                            start[1]=iterator_task_start_out[1];
...
//static herr_t  
static async_task_t * foreach_iter_task(async_instance_t *aid, H5VL_async_t *parent_obj, hid_t mem_type_id, hid_t mem_space_id,
...
{   
    ...
    return_task_val =foreach_iter_task(aid, parent_obj, mem_type_id, mem_space_id,
   ...
   // return_val =foreach_iter_task(aid, parent_obj, mem_type_id, mem_space_id,
    ...
    while(return_task_val!=NULL)
        {
            return_task_val =foreach_iter_task(aid, parent_obj, mem_type_id, mem_space_id,
                     file_space_id,plist_id, buf,return_task_val);
            print_dataspace(file_space_id);
        }
    //fprintf(stderr,"this is before H5Sget_select_type\n");
